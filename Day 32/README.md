# Day 32: TLS in Kubernetes MASTERCLASS | PART 2 | mTLS , kubeconfig & Kubernetes Context | CKA Certification
...
1. **Day 30**: How HTTPS & SSH Work, Encryption, and Its Types
   ...
   * **GitHub**: [Day 30 Repo](https://github.com/abchatterjee7/CKA-Certification/tree/main/Day%2030)
...
2. **Day 31 Part 1**: Mutual Auth (SSH), TLS 1.3, Types of Certificate Authorities (CAs)
   ...
   * **GitHub**: [Day 31 Part 1 Repo](https://github.com/abchatterjee7/CKA-Certification/tree/main/Day%2031%20Part%201)
...
| Add a new user                            | `kubectl config set-credentials aadi --client-certificate=aadi-cert.pem --client-key=aadi-key.pem --kubeconfig=~/kubeconfigs/seema-kubeconfig.yaml`                                   |
...
```bash
kubectl config set-credentials aadi \
  --client-certificate=~/kubeconfigs/aadi-cert.pem \
  --client-key=~/kubeconfigs/aadi-key.pem \
...
```bash
kubectl config set-context aadi@aws-cluster-context \
  --cluster=aws-cluster \
  --user=aadi \
  --namespace=default \
  --kubeconfig=~/kubeconfigs/my-2nd-kubeconfig-file
...
```bash
kubectl config use-context aadi@aws-cluster-context \
  --kubeconfig=~/kubeconfigs/my-2nd-kubeconfig-file
```
**Verify:**
